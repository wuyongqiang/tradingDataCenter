 ...
 * @since 1.0
 */
public class WeightedRouletteCrossoverOperator
  ...
  private WeightedRouletteSelector selector;
   ...
   * @since 2.0
   */
  public WeightedRouletteCrossoverOperator()
      throws InvalidConfigurationException {
   ...
   * @since 3.0
   */
  public WeightedRouletteCrossoverOperator(final Configuration a_configuration)
      throws InvalidConfigurationException {
  ...
  public void operate(final Population a_population,
    ...
    Population a_to_pop=null;
	try {
		a_to_pop = new Population(getConfiguration(),numCrossovers);
	} catch (InvalidConfigurationException e) {
		e.printStackTrace();
	}
	getSelector().empty();
    getSelector().select(numCrossovers, a_population, a_to_pop);
    //
    for (int i = 0; i < numCrossovers; i++) {
      index1 = i;
      IChromosome chrom1 = a_population.getChromosome(index1);

      IChromosome chrom2 = a_to_pop.getChromosome(i);
...
protected void doCrossover(IChromosome firstMate, IChromosome secondMate,
  ...
  public int compareTo(final Object a_other) {
    ...
    WeightedRouletteCrossoverOperator op = (WeightedRouletteCrossoverOperator) a_other;
  ...
  public boolean isXoverNewAge() {
	...
	public WeightedRouletteSelector getSelector() {
		if (selector == null){
			try {
				selector = new WeightedRouletteSelector(this.getConfiguration());
			} catch (InvalidConfigurationException e) {
				e.printStackTrace();
			} 
		}
